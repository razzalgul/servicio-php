...
echo "Iniciando carga de datos faltantes de Alimentacion Mina desde " . $inicio->format('Y-m-d H:i:s') . " hasta " . $fin->format('Y-m-d H:i:s') . PHP_EOL;
...
while ($current < $fin) {
    ...
    $queryNow = "SELECT Value FROM Runtime.dbo.AnalogHistory WHERE TagName = 'WCT5841WE120_002.Value' AND DateTime = '$end_str'";
    ...
    $queryPrev = "SELECT Value FROM Runtime.dbo.AnalogHistory WHERE TagName = 'WCT5841WE120_002.Value' AND DateTime = '$start_str'";
    ...
    if ($currentValue !== null && $previousValue !== null) {
        ...
        echo "Advertencia: No se encontró valor final o histórico para WCT5841WE120_002.Value en el rango $start_str a $end_str. Enviando 0." . PHP_EOL;
    }

    $plantFeed->quantity = $hourlyQuantity;
    $postData = json_encode($plantFeed);

    $url = "https://172.191.199.255/api/miningfeed/hour";
...
echo "Proceso de Alimentacion Mina finalizado." . PHP_EOL;
