...
$datos_general = array(
    ...
    'estado_general' => array(
        ...
        '5841_CB120.WIT' => 0, // balanza antigua
        'WIT841_002.IO.Value'=>0, // balanza nueva 
        'FIT0963.IO.Value' => 0,
        'FIT1401.IO.Value' => 0,
        'FIT1402.IO.Value' => 0,
        'FIT1801.IO.Value' => 0,
        ...
        'FIT2721.IO.Value' => 0,
        'FIT2722.IO.Value' => 0,
        'tonelajeFaja0Disenodia' => 0, // tonelaje de diseño dia faja 0 balanza nueva 
        'tonelajeFaja0DisenoTurno' => 0, // tonelaje de diseño turno faja 0 balanza nueva 
        'tonelajeFaja0ActualTurno' => 0, // tonelaje actual turno faja 0 balanza nueva 
        'tonelajeFaja0ActualDia'=>0,     // tonelaje actual dia faja 0 balanza nueva 
        'tonelajeFaja0DiaAnterior' => 0, // tonelaje del dia anterior faja 0 balanza nueva 
        'tonelajeFaja0TurnoAnterior'=>0, // tonelaje del turno anterior faja 0 balanza nueva 
        'diferenciaTonelajeFaja0Turno'=>0,
        'diferenciaTonelajeFaja0Dia'=>0,
        'tonelajedisenodia' => 0, // tonel1aje de diseño dia
    ...
    'circuito_4' => array(
        ...
        '5780_RS1001.Flow_CLD_2' => 0,
        '5780_RS2001.Flow_CLD_2' => 0,
        'WQI1801.Value' => 0,
        '5780_MX1001.status.Iavg' => 0,
        '5780_MX2001.status.Iavg' => 0,
        '5780_PU1001.Status.Current' => 0,
...
function consultabase(&$datos_produccion, &$inicioTurno, &$inicioTurnoAnterior, &$inicioDiaPrevio, &$inicioDia, &$datos_general)
{
 ...
 */
    ...
    $ton_envio_diseno = 0.5346; //metas del dia 46190 ----> x/24 ---> 1924.58 t/h --> 32.07 t/m --> 0.5346 t/s --->faja 5
    $ton_envio_faja0_diseno = 0.5469;// meta del dia 47250 ----- /24 ----1968.75 tn/h ---- faja 0- 
	...
	'VF2001.HZ2', 'VF2002.HZ2', 'VF2003.HZ2', 'VF2004.HZ2', 'VF2005.HZ2', 'VF2006.HZ2', 'VF2007.HZ2', 'VF2008.HZ2', 'VF2009.HZ2', 'VF2010.HZ2','5780_RS1001.Flow_CLD_2','5780_RS2001.Flow_CLD_2',
    ...
    $date_time = datetime::createfromformat('H:i:s',$dbtime);
    $date_time_formated = str_replace(array('am','pm'),array('a.m.','p.m.'), $date_time->format('H:i:s a'));
  

    $dato['queryTime'] = $date_time_formated;
    ...
    $queryInicioTurno = "SELECT TagName, round(Value,2) as pvalor from Runtime.dbo.AnalogHistory WHERE TagName IN ('WCT5841WE120_002.Value','WCT0303.Value','WCT1741.Value','WCT2741.Value') AND DateTime = '$inicioTurno'";

    $queryiniciTurnoAnterior = "SELECT TagName, round(Value,2) as pvalor from Runtime.dbo.AnalogHistory WHERE TagName IN ('WCT5841WE120_002.Value','WCT0303.Value','WCT1741.Value','WCT2741.Value') AND DateTime = '$inicioTurnoAnterior'";

    $queryInicioDiaPrevio = "SELECT TagName, round(Value,2) as pvalor from Runtime.dbo.AnalogHistory WHERE TagName IN ('WCT5841WE120_002.Value','WCT0303.Value','WCT1741.Value','WCT2741.Value') AND DateTime = '$inicioDiaPrevio'";
...
// liberamos los recursos y cerramos la conexiòn
  ...
  //calculamos tonelaje de diseno del  dia 
    $tonelajeDisenoDia = $segundosDia * $ton_produccion_diseno;
    $dato['tonelajedisenodia'] = $tonelajeDisenoDia;
    $tonelajeFaja0Disenodia = $segundosDia * $ton_envio_faja0_diseno;
    $dato['tonelajeFaja0Disenodia'] = $tonelajeFaja0Disenodia;
    $tonelajeEnvioDisenoDia = $segundosDia * $ton_envio_diseno;
    $dato['tonelajeenviodisenodia'] = $tonelajeEnvioDisenoDia;
    

    //  calculamos tonelaje de diseno del turno 
    $tonelajeDisenoTurno = $segundosTurno * $ton_produccion_diseno;
    $dato['tonelajedisenoTurno'] = $tonelajeDisenoTurno;
    $tonelajeFaja0DisenoTurno = $segundosTurno * $ton_envio_faja0_diseno;
    $dato['tonelajeFaja0DisenoTurno'] = $tonelajeFaja0DisenoTurno;
    $tonelajeEnvioDisenoTurno = $segundosTurno * $ton_envio_diseno;
    $dato['tonelajeenviodisenoturno'] = $tonelajeEnvioDisenoTurno;

    // calculmos el inicio del turno dependiendo de si es pasado las 8pm 
    $tonelajeInicioDia = (time() - $inicioDia >= 43200)? $datos_produccion['guardiaAnterior']: $datos_produccion['inicioGuardia'];

    // calculamos el tonelaje del dia anterior 

    $tonelajeDiaAnterior = ($tonelajeInicioDia['WCT1741.Value'] + $tonelajeInicioDia['WCT2741.Value']) - ($datos_produccion['guardiaDiaAnterior']['WCT1741.Value'] + $datos_produccion['guardiaDiaAnterior']['WCT2741.Value']);
    ...
    $tonelajeFaja0DiaAnterior =  $tonelajeInicioDia['WCT5841WE120_002.Value'] - $datos_produccion['guardiaDiaAnterior']['WCT5841WE120_002.Value'];
    $dato['tonelajeFaja0DiaAnterior'] = $tonelajeFaja0DiaAnterior;

    //calculamos tonelaje del dia  ----- 8am 8 am
    $tonelajeEnvioActualDia = $dato['WCT0303.Value'] - $tonelajeInicioDia['WCT0303.Value'];
    ...
    $tonelajeFaja0ActualDia = $dato['WCT5841WE120_002.Value'] - $tonelajeInicioDia['WCT5841WE120_002.Value'];
    $dato['tonelajeFaja0ActualDia'] = $tonelajeFaja0ActualDia;

    //calculamos tonelaje de la guardia anterior 
    ...
    $tonelajeFaja0TurnoAnterior = $datos_produccion['inicioGuardia']['WCT5841WE120_002.Value'] - $datos_produccion['guardiaAnterior']['WCT5841WE120_002.Value'];
    $dato['tonelajeFaja0TurnoAnterior'] = $tonelajeFaja0TurnoAnterior;

    // calculamos tonelaje del turno actual 
    ...
    $tonelajeFaja0ActualTurno = $dato['WCT5841WE120_002.Value'] - $datos_produccion['inicioGuardia']['WCT5841WE120_002.Value'];
    $dato['tonelajeFaja0ActualTurno'] = $tonelajeFaja0ActualTurno;

    // calculamos la diferencia de turno 
    ...
    $diferenciaTonelajeFaja0Turno = $tonelajeFaja0ActualTurno - $tonelajeFaja0DisenoTurno;
    $dato['diferenciaTonelajeFaja0Turno'] = $diferenciaTonelajeFaja0Turno;



    // calculamos la diferencia del dia diferencia dia 
    ...
    $diferenciaTonelajeFaja0Dia =$tonelajeFaja0ActualDia - $tonelajeFaja0Disenodia;
    $dato['diferenciaTonelajeFaja0Dia'] = $diferenciaTonelajeFaja0Dia;
    ...
    for ($i = 1; $i < 11; $i++) {
        ...
        if ($dato["VF10{$numeroFiltro}.HZ1"] > 0 && $dato["VF10{$numeroFiltro}.HZ2"] > 0) {
            $filtros_l1 = $filtros_l1 + 1;
        }
        if ($dato["VF20{$numeroFiltro}.HZ1"] > 0 && $dato["VF20{$numeroFiltro}.HZ2"] > 0) {
    ...
    print_r($datos_general['estado_general']);
...
co::run(function () {
    ...
    while (1) {
          ...
          echo "mensaje de recibida es".$recibido;
            $cli_internet->push(json_encode($data_registro));
           ...
           echo "mensaje de recibida es".$recibido;
            ...
            $intento = 0;
            while (1){
        $dato = consultabase($datos_produccion, $inicioTurno, $inicioTurnoAnterior, $inicioDiaPrevio, $inicioDia, $datos_general);
               // sendToFirebase($firebasedata);
                $dato['tipo_operacion'] = "refresco";
                $dato['tipo_usuario'] = "localbd";
                //var_dump($dato);
                $cli_internet->push(json_encode($dato));
        
                //    echo microtime(true).PHP_EOL;
                $recibido = $cli_internet->recv(4);
                echo $recibido;
                if (strpos($recibido, "mensaje")) {
                    ...
                    while ($intento <= 5) {
                        ...
                        $ret_internet = $cli_internet->upgrade("/wss/");
